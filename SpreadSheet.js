...
function SpreadSheet(parentElement, options) {
	...
	SpreadSheet.prototype.createEmptySheet = function() {
		...
		let _emptyTable = DOMManager.createEmptyTable(_rows, _columns);
		_emptyTable.classList.add("SpreadSheet--empty_table");
		_dataTables.push(_emptyTable);

		_dataTablesChangedEvent = new CustomEvent('dataTablesChanged', {
			detail: this, 
		});
		document.dispatchEvent(_dataTablesChangedEvent);
		_dataContainer.appendChild(_emptyTable);
	}


	SpreadSheet.prototype.createSheetFromData = function(data) {
		this.createVerticalHeading();
		this.createHorizontalHeading();
		...
		_container.style.cssText = "width: 3634px; height: 1050px; position: relative; left: 22px; top: 0; overflow: hidden";
	...
	SpreadSheet.prototype.createHorizontalHeading = function() {
	...
	SpreadSheet.prototype.createVerticalHeading = function() {
		// if querySelector.... not exists....
		let _data = [];
		let _columns = 1;
